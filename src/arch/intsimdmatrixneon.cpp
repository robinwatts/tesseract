...
PartialMatrixDotVector8(const int8_t* __restrict wi,
  ...
  }
  {
    ...
    *v++ = (static_cast<double>(vget_lane_s32(vget_low_s32 (result0123), 0))) * *scales++;
    if (num_out > 1)
      *v++ = (static_cast<double>(vget_lane_s32(vget_low_s32 (result0123), 1))) * *scales++;
    if (num_out > 2)
      *v++ = (static_cast<double>(vget_lane_s32(vget_high_s32(result0123), 0))) * *scales++;
    if (num_out > 3)
      *v++ = (static_cast<double>(vget_lane_s32(vget_high_s32(result0123), 1))) * *scales++;
    if (num_out > 4)
      *v++ = (static_cast<double>(vget_lane_s32(vget_low_s32 (result4567), 0))) * *scales++;
    if (num_out > 5)
      *v++ = (static_cast<double>(vget_lane_s32(vget_low_s32 (result4567), 1))) * *scales++;
    if (num_out > 6)
      *v++ = (static_cast<double>(vget_lane_s32(vget_high_s32(result4567), 0))) * *scales++;
    if (num_out > 7)
      *v   = (static_cast<double>(vget_lane_s32(vget_high_s32(result4567), 1))) * *scales;
  }
}
